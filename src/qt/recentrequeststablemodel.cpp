    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return InfinitumUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, InfinitumUnits::separatorNever);
            else
                return InfinitumUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+InfinitumUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
