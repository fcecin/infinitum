...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CInfinitumAddress(CScriptID(redeemScript)).ToString();
...
CAmount CWallet::GetCredit(const CTxOut& txout, uint64_t nCoinHeight, const isminefilter& filter) const
{
    ...
    // Infinitum:: zero value from a txout that cannot be spent at the given height
    if (nVal > 0 && nCoinHeight != 0xFFFFFFFFFFFFFFFF && IsOutputPruned(txout.nValue, nCoinHeight))
        return 0;
    return nVal;    
}
                           ...
                           list<COutputEntry>& listSent, CAmount& nFee, string& strSentAccount, const isminefilter& filter) const
{
	...
	// Infinitum:: outputs that have been pruned don't exist.
	if (IsOutputPruned(txout.nValue, nCoinHeight))
	    continue;
...
CAmount CWalletTx::GetCredit(const isminefilter& filter) const
{
    ...
    // Infinitum:: turn off the cache (FIXME/REVIEW: do we really need to kill it?)
    fCreditCached = false;
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeZeroValue) const
{
...

    {
	    ...
	    int nCoinHeight = pcoin->GetHeightInMainChainOrTipHeight();
		    ...
		    // Infinitum:: output is unavailable if it is pruned for inactivity or dust
		    && (!IsOutputPruned(pcoin->vout[i].nValue, nCoinHeight))
		    )
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-infinitum-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CInfinitumAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CInfinitumAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CInfinitumAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CInfinitumAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CInfinitumAddress(address).ToString());
}
...
std::map<CTxDestination, CAmount> CWallet::GetAddressBalances()
{
...

    {
		...
		// Infinitum:: unexistent output if it has been pruned
		if (IsOutputPruned(pcoin->vout[i].nValue, nCoinHeight))
		    continue;
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CInfinitumAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CInfinitumAddress(dest).ToString(), key);
}
