...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s infinitum-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  infinitum-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded infinitum transaction") + "\n" +
              "  infinitum-tx [options] -create [commands]   " + _("Create hex-encoded infinitum transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const string& strInput)
{
    ...
    CInfinitumAddress addr(strAddr);
    if (!addr.IsValid())
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded infinitum transaction
