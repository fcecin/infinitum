...
###   This script attempts to download the signature file SHA256SUMS.asc from infinitum.org
...
WORKINGDIR="/tmp/infinitum"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://infinitum.org/bin/"
VERSIONPREFIX="infinitum-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'infinitum-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Infinitum Core binary release signing key installed?"
   fi
