...
The RPM spec file provided here is for Infinitum-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://infinitum.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Infinitum git repository](https://github.com/infinitum/infinitum) but are not part of
...
Currently only `Source10` is used, for the example `infinitum.conf` file.
...
in the infinitum git repository. At present only an SVG version of the Infinitum
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the infinitum
...
needed when 0.12.1 is released, a proper fix is already in the Infinitum git
master branch.
...
idea to build Infinitum Core against a static version of that library compiled
...
The `infinitum.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `infinitum-server` package. At this point, most RPM based
...
The `infinitum-server` package should have SELinux support. How to properly do
...
Infinitum-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/infinitum/). He has been
packaging Infinitum for Fedora at least since 2012.
...
4. I renamed the `infinitum` package that contains the Qt GUI to `infinitum-core` as
that appears to be how the general population refers to it, in contrast to
`infinitum-xt` or `infinitum-classic`. I wanted to make sure the general population
